...
scheduler.add_job(aggiorna_cambi, 'cron', hour=12,  minute=0)
scheduler.add_job(aggiorna_cambi, 'cron', hour=20, minute=0)
scheduler.start()
...
def update():
    ...
    try:
        aggiorna_cambi()
        return {"status": "ok", "message": "Aggiornamento completato"}
...
def get_rates(
    ...
    if row:
        ...
        # Inferenza cross rate: BASE/QUOTE = (EUR/QUOTE) / (EUR/BASE)
